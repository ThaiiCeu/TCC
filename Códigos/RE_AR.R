...
adf.test(RE$Arábica) # É Estacionária

VO

model = lm(RE$Arábica ~ RE$`Mês/Ano`) # Aqui está nos afirmando que é estacionária
summary(model)
...
RE_AR <- RE %>% select(Arábica)
RE_AR <- ts(RE_AR, start = c(2015, 1), frequency = 96)

# Gerar as harmônicas
harmonicas <- fourier(RE_AR, K = K)

# Ajustar o modelo inicial
modelo_inicial <- tslm(RE_AR ~ I(trend^2) + I(trend^3) + harmonicas)
...
modelo_final_harmonica <- tslm(RE_AR ~ I(trend^2) + I(trend^3) + harmonicas_significativas)
...
dados_harmonica <- data.frame(
  ...
  Observado = RE$Arábica,      # remove os dois primeiros
...
mod_co_REAR <- cochrane.orcutt(modelo_final_harmonica)

resumo <- data.frame(
  Coeficiente = mod_co_REAR$coefficients,
  `Erro Padrão` = mod_co_REAR$std.error,
  `t valor` = mod_co_REAR$t.value,
  `p valor` = mod_co_REAR$p.value
)

print(round(resumo, 4))

mod_co_REAR$r.squared

res <- mod_co_REAR$residuals
...
valores_ajustados_harmonica_co = mod_co_REAR$fitted.values

# Criar um data frame com os dados
dados_harmonica <- data.frame(
  Tempo = RE$`Mês/Ano`, 
  Observado = RE$Arábica,
...
dummies_mes <- seasonaldummy(RE_AR)  # Cria dummies sazonais (1 para cada mês)

# Ajustar o modelo inicial com todas as dummies
modelo_inicial_dummies <- tslm(RE_AR ~ dummies_mes)
...
# Ajustar o modelo com as harmônicas significativas
modelo_final_dummies <- tslm(RE_AR ~  dummies_significativas)
