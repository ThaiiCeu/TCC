...
modelo_inicial <- tslm(VO_AR ~ trend + I(trend^2) + I(trend^3) +  harmonicas)
...
modelo_final_harmonica <- tslm(VO_AR ~ trend + I(trend^2) + I(trend^3) + harmonicas_significativas)
...
shapiro.test(mod_co_VOAR$residuals)

dwtest(modelo_final_harmonica)

# Transformação Orkutt
mod_co_VOAR <- cochrane.orcutt(modelo_final_harmonica)

resumo <- data.frame(
  Coeficiente = mod_co_VOAR$coefficients,
  `Erro Padrão` = mod_co_VOAR$std.error,
  `t valor` = mod_co_VOAR$t.value,
  `p valor` = mod_co_VOAR$p.value
)
...
valores_ajustados_harmonica_co = mod_co_VOAR$fitted.values
...
# Ajustar o modelo com as harmônicas significativas
modelo_final_dummies <- tslm(VO_AR ~ trend + I(trend^2) + dummies_significativas)
