...
adf.test(VO$Conillon) # Não É Estacionária

VO

model = lm(VO$Conillon ~ VO$`Mês/Ano`) # Aqui está nos afirmando que não é estacionária
summary(model)

# Número de harmônicas
K <- 6  

# Criar a dummy do intervalo (2016-01 a 2018-12)
VO$dummy_decaimento <- ifelse(
  VO$`Mês/Ano` >= as.Date("2016-01-01") & VO$`Mês/Ano` <= as.Date("2018-05-01"),
  1, 0
)

# Transformar a série em ts
VO_CO <- VO %>% select(Conillon)
VO_CO <- ts(VO_CO, start = c(2015, 1), frequency = 96)

# Gerar as harmônicas
harmonicas <- fourier(VO_CO, K = K)

# Ajustar o modelo inicial
modelo_inicial <- tslm(VO_CO ~  harmonicas )
...
modelo_final_harmonica <- tslm(VO_CO ~  harmonicas_significativas + VO$dummy_decaimento)
...
dados_harmonica <- data.frame(
  Tempo = VO$`Mês/Ano`,       # remove o primeiro valor
  Observado = VO$Conillon,    # remove o primeiro valor
...
mod_co_VOCO <- cochrane.orcutt(modelo_final_harmonica)

resumo <- data.frame(
  Coeficiente = mod_co_VOCO$coefficients,
  `Erro Padrão` = mod_co_VOCO$std.error,
  `t valor` = mod_co_VOCO$t.value,
  `p valor` = mod_co_VOCO$p.value
)
...
valores_ajustados_harmonica_co = mod_co_VOCO$fitted.values

# Criar um data frame com os dados
dados_harmonica <- data.frame(
  Tempo = VO$`Mês/Ano`, 
  Observado = VO$Conillon,
...
mod_co_VOCO$r.squared


#########modelo_final_harmonica############################### MODELO DUMMIES
# Criar variáveis dummy para os meses
dummies_mes <- seasonaldummy(VO_CO)  # Cria dummies sazonais (1 para cada mês)

# Ajustar o modelo inicial com todas as dummies
modelo_inicial_dummies <- tslm(VO_CO ~ trend + dummies_mes)
...
# Ajustar o modelo com as harmônicas significativas
modelo_final_dummies <- tslm(VO_CO ~ trend + dummies_significativas)
