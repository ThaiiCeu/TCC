...
adf.test(VO$Solúvel) # É Estacionária

VO

model = lm(VO$Solúvel ~ VO$`Mês/Ano`) # Aqui está nos afirmando que é estacionária
summary(model)
...
VO_SO <- VO %>% select(Solúvel)
VO_SO <- ts(VO_SO, start = c(2015, 1), frequency = 12)

# Gerar as harmônicas
harmonicas <- fourier(VO_SO, K = K)

# Ajustar o modelo inicial
modelo_inicial <- tslm(VO_SO ~ trend +  harmonicas)
...
modelo_final_harmonica <- tslm(VO_SO ~ trend + harmonicas_significativas)
...
mod_co_VOSO <- cochrane.orcutt(modelo_final_harmonica)

resumo <- data.frame(
  Coeficiente = mod_co_VOSO$coefficients,
  `Erro Padrão` = mod_co_VOSO$std.error,
  `t valor` = mod_co_VOSO$t.value,
  `p valor` = mod_co_VOSO$p.value
)
...
valores_ajustados_harmonica_co = mod_co_VOSO$fitted.values
