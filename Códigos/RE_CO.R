...
adf.test(RE$Conillon) # É Estacionária

VO

model = lm(RE$Conillon ~ RE$`Mês/Ano`) # Aqui está nos afirmando que é estacionária
summary(model)
...
RE_CO <- RE %>% select(Conillon)
RE_CO <- ts(RE_CO, start = c(2015, 1), frequency = 96)

# Gerar as harmônicas
harmonicas <- fourier(RE_CO, K = K)

# Ajustar o modelo inicial
modelo_inicial <- tslm(RE_CO ~ I(trend^2) + I(trend^3) + I(trend^4) + I(trend^5) + I(trend^6) + I(trend^7) +  harmonicas)
...
modelo_final_harmonica <- tslm(RE_CO ~ I(trend^2) + I(trend^3) + I(trend^4) + I(trend^5) + I(trend^6) + I(trend^7)  + harmonicas_significativas)
...
dados_harmonica <- data.frame(
  Tempo = RE$`Mês/Ano`,        # remove os dois primeiros
  Observado = RE$Conillon,      # remove os dois primeiros
...
mod_co_RECO <- cochrane.orcutt(modelo_final_harmonica)

resumo <- data.frame(
  Coeficiente = mod_co_RECO$coefficients,
  `Erro Padrão` = mod_co_RECO$std.error,
  `t valor` = mod_co_RECO$t.value,
  `p valor` = mod_co_RECO$p.value
)

print(round(resumo, 4))

mod_co_RECO$r.squared

res <- mod_co_RECO$residuals
...
valores_ajustados_harmonica_co = mod_co_RECO$fitted.values
# Criar um data frame com os dados
dados_harmonica <- data.frame(
  Tempo = RE$`Mês/Ano`, 
  Observado = RE$Conillon,
...
dummies_mes <- seasonaldummy(RE_CO)  # Cria dummies sazonais (1 para cada mês)

# Ajustar o modelo inicial com todas as dummies
modelo_inicial_dummies <- tslm(RE_CO ~ dummies_mes)
...
# Ajustar o modelo com as harmônicas significativas
modelo_final_dummies <- tslm(RE_CO ~  dummies_significativas)
