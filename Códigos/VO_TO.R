...
adf.test(VO$Torrado) # É Estacionária

VO

model = lm(VO$Torrado ~ VO$`Mês/Ano`) # Aqui está nos afirmando que é estacionária
summary(model)
...
VO_TO <- VO %>% select(Torrado)
VO_TO <- ts(VO_TO, start = c(2015, 1), frequency = 96)

# Gerar as harmônicas
harmonicas <- fourier(VO_TO, K = K)

# Ajustar o modelo inicial
modelo_inicial <- tslm(VO_TO ~ harmonicas)
...
modelo_final_harmonica <- tslm(VO_TO ~ harmonicas_significativas)
...
dados_harmonica <- data.frame(
  ...
  Observado = VO$Torrado,    # remove o primeiro valor
...
ggplot(dados_long_harmonica, aes(x = Tempo, y = Valor, color = Série)) +
  ...
  labs(
    title = "Série Temporal: Dados Observados vs. Modelo Ajustado (Torrado)",
    x = "Tempo",
    y = "Valor",
    color = "Legenda"
  ) +
...
mod_co_VOTO <- cochrane.orcutt(modelo_final_harmonica)

resumo <- data.frame(
  Coeficiente = mod_co_VOTO$coefficients,
  `Erro Padrão` = mod_co_VOTO$std.error,
  `t valor` = mod_co_VOTO$t.value,
  `p valor` = mod_co_VOTO$p.value
)
...
valores_ajustados_harmonica_co = mod_co_VOTO$fitted.values

# Criar um data frame com os dados
dados_harmonica <- data.frame(
  Tempo = VO$`Mês/Ano`, 
  Observado = VO$Torrado,
...
dummies_mes <- seasonaldummy(VO_TO)  # Cria dummies sazonais (1 para cada mês)

# Ajustar o modelo inicial com todas as dummies
modelo_inicial_dummies <- tslm(VO_TO ~ trend + dummies_mes)
...
# Ajustar o modelo com as harmônicas significativas
modelo_final_dummies <- tslm(VO_TO ~  dummies_significativas)
...
dados_dummies <- data.frame(
  Tempo = VO$`Mês/Ano`[-1],       # remove o primeiro valor
  Observado = TO,    # remove o primeiro valor
