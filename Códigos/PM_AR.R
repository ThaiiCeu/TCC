...
adf.test(PM$Arábica) # Não É Estacionária

VO

model = lm(PM$Arábica ~ VO$`Mês/Ano`) # Aqui está nos afirmando que não é estacionária
summary(model)
...
PM_AR <- PM %>% select(Arábica)
PM_AR <- ts(PM_AR, start = c(2015, 1), frequency = 96)

# Gerar as harmônicas
harmonicas <- fourier(PM_AR, K = K)

# Ajustar o modelo inicial
modelo_inicial <- tslm(PM_AR ~ trend + harmonicas) 
...
modelo_final_harmonica <- tslm(PM_AR ~ trend +  harmonicas_significativas)
...
dados_harmonica <- data.frame(
  Tempo = PM$`Mês/Ano`,        # remove os dois primeiros
  Observado = PM$Arábica,      # remove os dois primeiros
...
ggplot(dados_long_harmonica, aes(x = Tempo, y = Valor, color = Série)) +
  ...
  labs(
    title = "Série Temporal: Dados Observados vs. Modelo Ajustado (Arábica)",
    x = "Tempo",
    y = "Valor",
    color = "Legenda"
  ) +
...
mod_co_PMAR <- cochrane.orcutt(modelo_final_harmonica)

resumo <- data.frame(
  Coeficiente = mod_co_PMAR$coefficients,
  `Erro Padrão` = mod_co_PMAR$std.error,
  `t valor` = mod_co_PMAR$t.value,
  `p valor` = mod_co_PMAR$p.value
)

print(round(resumo, 4))

mod_co_PMAR$r.squared

res <- modelo_final_harmonica$residuals
...
valores_ajustados_harmonica_co = mod_co_PMAR$fitted.values

# Criar um data frame com os dados
dados_harmonica <- data.frame(
  Tempo = PM$`Mês/Ano`, 
  Observado = PM$Arábica,
