...
adf.test(PM$Torrado) # É Estacionária

VO

model = lm(PM$Torrado ~ PM$`Mês/Ano`) # Aqui está nos afirmando que é estacionária
summary(model)
...
PM_TO <- PM %>% select(Torrado)
PM_TO <- ts(PM_TO, start = c(2015, 1), frequency = 48)

# Gerar as harmônicas
harmonicas <- fourier(PM_TO, K = K)

# Ajustar o modelo inicial
modelo_inicial <- tslm(PM_TO ~ I(trend^2) + harmonicas)

# Resumo do modelo
summary(modelo_inicial) # Nenhuma harmônica significativa 
...
modelo_final_harmonica <- tslm(PM_TO ~ I(trend^2) +  harmonicas_significativas)
...
dados_harmonica <- data.frame(
  ...
  Observado = PM$Torrado,    # remove o primeiro valor
...
mod_co_PMTO <- cochrane.orcutt(modelo_final_harmonica)

resumo <- data.frame(
  Coeficiente = mod_co_PMTO$coefficients,
  `Erro Padrão` = mod_co_PMTO$std.error,
  `t valor` = mod_co_PMTO$t.value,
  `p valor` = mod_co_PMTO$p.value
)

print(round(resumo, 4))

mod_co_PMTO$r.squared

res <- mod_co_PMTO$residuals
...
valores_ajustados_harmonica_co = mod_co_PMTO$fitted.values

# Criar um data frame com os dados
dados_harmonica <- data.frame(
  Tempo = PM$`Mês/Ano`, 
  Observado = PM$Torrado,
...
dummies_mes <- seasonaldummy(VO_SO)  # Cria dummies sazonais (1 para cada mês)

# Ajustar o modelo inicial com todas as dummies
modelo_inicial_dummies <- tslm(VO_SO ~ trend + dummies_mes)
