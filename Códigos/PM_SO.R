...
adf.test(PM$Solúvel) # É Estacionária

PM

model = lm(PM$Solúvel ~ PM$`Mês/Ano`) # Aqui está nos afirmando que é estacionária
summary(model)
...
PM_SO <- PM %>% select(Solúvel)
PM_SO <- ts(PM_SO, start = c(2015, 1), frequency = 12)

# Gerar as harmônicas
harmonicas <- fourier(PM_SO, K = K)

# Ajustar o modelo inicial
modelo_inicial <- tslm(PM_SO ~ trend +  I(trend^2) + I(trend^3) + I(trend^4) + I(trend^5) + I(trend^6) + I(trend^7) + I(trend^8) + harmonicas)
...
modelo_final_harmonica <- tslm(PM_SO ~ I(trend^2) + I(trend^3) + I(trend^4) + I(trend^5) + I(trend^6) + I(trend^7) + I(trend^8))
...
dados_harmonica <- data.frame(
  Tempo = PM$`Mês/Ano`,       # remove o primeiro valor
  Observado = PM$Solúvel,    # remove o primeiro valor
...
mod_co_PMSO <- cochrane.orcutt(modelo_final_harmonica)

resumo <- data.frame(
  Coeficiente = mod_co_PMSO$coefficients,
  `Erro Padrão` = mod_co_PMSO$std.error,
  `t valor` = mod_co_PMSO$t.value,
  `p valor` = mod_co_PMSO$p.value
)

print(round(resumo, 4))

mod_co_PMSO$r.squared

res <- mod_co_PMSO$residuals
...
valores_ajustados_harmonica_co = mod_co_PMSO$fitted.values

# Criar um data frame com os dados
dados_harmonica <- data.frame(
  Tempo = PM$`Mês/Ano`, 
  Observado = PM$Solúvel,
...
ggplot(dados_long_harmonica, aes(x = Tempo, y = Valor, color = Série)) +
  ...
  scale_color_manual(values = c("red", "black")) +
  labs(
    title = "Série Temporal: Dados Observados vs. Modelo Ajustado (Solúvel)",
    x = "Tempo",
    y = "Valor",
    color = "Legenda"
  ) +
...
dummies_mes <- seasonaldummy(PM_SO)  # Cria dummies sazonais (1 para cada mês)

# Ajustar o modelo inicial com todas as dummies
modelo_inicial_dummies <- tslm(PM_SO ~ trend + dummies_mes)
...
# Ajustar o modelo com as harmônicas significativas
modelo_final_dummies <- tslm(VO_SO ~ trend + I(trend^2) + dummies_significativas)
