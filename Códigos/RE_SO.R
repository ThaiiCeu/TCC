...
adf.test(RE$Solúvel) # É Estacionária

VO

model = lm(RE$Solúvel ~ RE$`Mês/Ano`) # Aqui está nos afirmando que é estacionária
summary(model)
...
RE_SO <- RE %>% select(Solúvel)
RE_SO <- ts(RE_SO, start = c(2015, 1), frequency = 96)

# Gerar as harmônicas
harmonicas <- fourier(RE_SO, K = K)

# Ajustar o modelo inicial
modelo_inicial <- tslm(RE_SO ~ harmonicas)
...
modelo_final_harmonica <- tslm(RE_SO ~ harmonicas_significativas)
...
dados_harmonica <- data.frame(
  ...
  Observado = RE$Solúvel,      # remove os dois primeiros
...
mod_co_RESO <- cochrane.orcutt(modelo_final_harmonica)

resumo <- data.frame(
  Coeficiente = mod_co_RESO$coefficients,
  `Erro Padrão` = mod_co_RESO$std.error,
  `t valor` = mod_co_RESO$t.value,
  `p valor` = mod_co_RESO$p.value
)

print(round(resumo, 4))

mod_co_RESO$r.squared

res <- mod_co_RESO$residuals
...
valores_ajustados_harmonica_co = mod_co_RESO$fitted.values

# Criar um data frame com os dados
dados_harmonica <- data.frame(
  Tempo = RE$`Mês/Ano`, 
  Observado = RE$Solúvel,
...
dummies_mes <- seasonaldummy(RE_SO)  # Cria dummies sazonais (1 para cada mês)

# Ajustar o modelo inicial com todas as dummies
modelo_inicial_dummies <- tslm(RE_SO ~ dummies_mes)
