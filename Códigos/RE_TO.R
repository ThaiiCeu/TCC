...
adf.test(RE$Torrado) # É Estacionária

VO

model = lm(RE$Torrado ~ RE$`Mês/Ano`) # Aqui está nos afirmando que é estacionária
summary(model)
...
RE_TO <- RE %>% select(Torrado)
RE_TO <- ts(RE_TO, start = c(2015, 1), frequency = 96)

# Gerar as harmônicas
harmonicas <- fourier(RE_TO, K = K)

# Ajustar o modelo inicial
modelo_inicial <- tslm(RE_TO ~  I(trend^5)  + harmonicas)
...
modelo_final_harmonica <- tslm(RE_TO ~  I(trend^5) + harmonicas_significativas)
...
dados_harmonica <- data.frame(
  ...
  Observado = RE$Torrado,      # remove os dois primeiros
...
mod_co_RETO <- cochrane.orcutt(modelo_final_harmonica)

resumo <- data.frame(
  Coeficiente = mod_co_RETO$coefficients,
  `Erro Padrão` = mod_co_RETO$std.error,
  `t valor` = mod_co_RETO$t.value,
  `p valor` = mod_co_RETO$p.value
)

print(round(resumo, 4))

mod_co_RETO$r.squared

res <- mod_co_RETO$residuals
...
valores_ajustados_harmonica_co = mod_co_RETO$fitted.values

# Criar um data frame com os dados
dados_harmonica <- data.frame(
  Tempo = RE$`Mês/Ano`, 
  Observado = RE$Torrado,
...
dummies_mes <- seasonaldummy(RE_TO)  # Cria dummies sazonais (1 para cada mês)

# Ajustar o modelo inicial com todas as dummies
modelo_inicial_dummies <- tslm(RE_TO ~ dummies_mes)
...
# Ajustar o modelo com as harmônicas significativas
modelo_final_dummies <- tslm(RE_SO ~  dummies_significativas)
